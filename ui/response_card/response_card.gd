extends Control
## Adapted from D:\GitHub\the-spire\Scripts\CardNode.gd. Behavior is unchanged --
## Dictionary-driven setup(), affordability tinting, hover/click, card_played signal.
## The only change: $Bg is a plain ColorRect instead of a NinePatchRect bound to real
## card art, and there are no custom font dependencies, per "use placeholder assets."
...
@onready var _card_category: Label = $CardCategory
...
func setup(data: Dictionary) -> void:
	...
	_type_tint.color = TYPE_COLORS.get(str(data.get("type", "")), Color(0.5, 0.5, 0.5, 0.3))
...
func _on_mouse_entered() -> void:
	if _bg:
		_bg.modulate = Color(1.12, 1.12, 1.05)
